...
public protocol EventPublisher {
    ...
    /// - returns: `Self`
    func onPlaybackCreated(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    
    /// Published when the associated media has loaded but is not playback ready.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackPrepared(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    ...
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackReady(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    
    /// Published once the playback first starts. This should be a one-time-event.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackStarted(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    
    /// Published when playback is paused for some reason.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackPaused(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    
    /// Published when playback is resumed from a paused state for some reason.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackResumed(callback: @escaping (Tech, Context.Source) -> Void) -> Self

    /// Published when playback is stopped by user action.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackAborted(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    
    /// Published when playback reached the end of the current media, ie when playback reaches `duration`.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackCompleted(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    ...
    /// - returns: `Self`
    func onError(callback: @escaping (Tech?, Context.Source?, Context.ContextError) -> Void) -> Self
    ...
    /// - returns: `Self`
    func onBitrateChanged(callback: @escaping (Tech, Context.Source, Double) -> Void) -> Self
    
    /// Published once buffering started.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onBufferingStarted(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    
    /// Published once buffering stopped.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onBufferingStopped(callback: @escaping (Tech, Context.Source) -> Void) -> Self
    
    /// Published when user scrubs in the player.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onPlaybackScrubbed(callback: @escaping (Tech, Context.Source, Int64) -> Void) -> Self
    
    /// Published if the current playback `duration` changed.
    ///
    /// - parameter callback: callback to fire once the event is fired.
    /// - returns: `Self`
    func onDurationChanged(callback: @escaping (Tech, Context.Source) -> Void) -> Self
}
